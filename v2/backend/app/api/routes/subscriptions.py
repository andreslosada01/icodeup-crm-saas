...
from app.models import SaasPlan, Tenant, TenantModule, TenantSubscription, User
from app.schemas.subscription import SaasPlanCreate, SaasPlanOut, TenantModuleIn, TenantModuleOut, TenantSubscriptionOut, TenantSubscriptionUpsert


router = APIRouter()

SUPPORTED_MODULES = {"core", "administration", "collections", "legal", "sales", "documents", "bi", "integrations"}
...
def module_enabled_for_tenant(db: Session, tenant_id: int, module_code: str) -> bool:
    ...
    return True if module is None else module.enabled
...
def update_tenant_modules(tenant_id: int, payload: list[TenantModuleIn], db: Session = Depends(get_db), user: User = Depends(current_user)) -> list[TenantModule]:
    ...
    for item in payload:
        ...
        if module is None:
            module = TenantModule(tenant_id=tenant_id, module_code=item.module_code, enabled=item.enabled)
            db.add(module)
        else:
